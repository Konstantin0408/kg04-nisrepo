...
void sighandle(int nsig) {
  if (nsig == SIGINT) {
    ...
    if (send(clntSock3, exitStr, RCVBUFSIZE, 0) != RCVBUFSIZE)
            DieWithError("send() failed");
    if (send(dsplSock,  exitStr, RCVBUFSIZE, 0) != RCVBUFSIZE)
            DieWithError("send() failed");
...
int main(int argc, char *argv[])
{
    ...
    struct sockaddr_in echoDsplAddr; /* Displayer address */
    ...
    if (listen(servSock, MAXPENDING) < 0)
        ...
        if ((dsplSock = accept(servSock, (struct sockaddr *) &echoDsplAddr,
                               &clntLen)) < 0)
            DieWithError("accept() failed");
  ...
  int socks[4] = { clntSock1, clntSock2, clntSock3, dsplSock };
  pid1 = fork();
  if (pid1 == 0) {
    // child 1
    srand(time(0) + 0);
