...
void displayInfo(char id, int arg1, char *arg2, int arg3) {
  char str[RCVBUFSIZE];
  str[0] = (char)3;
  str[1] = (char)30;
  str[2] = id;
  *((int*)(str + 3)) = arg1;
  ...
  char *p = (char*)((int*)(str + 3) + 1);
  if (arg2) for (int i = 0; i < 10; ++i) p[i] = arg2[i];
  *((int*)(p + 10)) = arg3;
  if (send(serverSock, str, RCVBUFSIZE, 0) != RCVBUFSIZE)
...
void proger(int num, int sock) {
  ...
  displayInfo(1, num, my_prog, checker);
  ...
  while (true) {
    ...
    if (query_type == tocheck_type) {
      ...
      if (checking_prog[0] <= 'f' || checking_prog[1] <= 'f') {
        ...
        displayInfo(4, num, checking_prog, 0);
        CRIT_BEGIN
        resetBuffer();
        bufferToEcho(&author, 1);
